...
class sliding_elastic:
    ...
    def __init__(
        ...
        self.atr.addAttrib(
            _var("shell_bottom_primary", shell_bottom_primary, attributes)
        )
        self.atr.addAttrib(
            _var("shell_bottom_secondary", shell_bottom_secondary, attributes)
        )

    def tree(self):
        tree = ET.Element(
            "contact", type=self.type, name=self.name, surface_pair=self.surface_pair
        )
